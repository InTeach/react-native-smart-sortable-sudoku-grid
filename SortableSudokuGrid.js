...
class SortableSudokuGrid extends Component {
  ...
  constructor(props) {
    ...
    let containerHeight =
      dataSource.length > 0
        ? (Math.floor((dataSource.length - 1) / columnCount) + 1) *
          this._rowHeight
        : this._rowHeight;
        ...
        <SortableCell
          ref={component => {
            ...
            if (
              ...
              this._cells.length > dataList.length
            ) {
              this._cells.splice(
                index + 1,
                this._cells.length - dataList.length
  ...
  _sortCells = ({ cellsAnimationOptions, callback }) => {
    ...
    callback &&
      Animated.parallel(animationParallels).start(() => {
        callback && callback();
      });
  };
  ...
  removeCell = ({ cellIndex, callback }) => {
    ...
    if (currentCellIndex < this._cells.length - 1) {
      ...
      //removing last cell do not need to sort cell
  ...
  _updateContainerHeight = ({
    ...
    let newDataSourceLength =
      oldDataSourceLength + (cellChangeType == cellChangeTypes.add ? 1 : -1);
    //if (oldDataSourceLength == 0 || newDataSourceLength == 0) {
    if (
      cellChangeType == cellChangeTypes.remove &&
      (oldDataSourceLength == 0 || newDataSourceLength == 0)
    ) {
      return;
    }
    let { columnCount } = this.props;
    ...
    let height =
      (Math.floor((newDataSourceLength - 1) / columnCount) + 1) *
      this._rowHeight;
    ...
    this._animationInstace = Animated.timing(this.state.containerHeight, {
      toValue: height,
